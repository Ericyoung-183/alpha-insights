...
![Alpha Insights](assets/poster_zh.png)
...
| **小红书数据** | 消费者舆情、产品反馈、趋势洞察 | 内置脚本。需配置 TikHub API Key，创建 `~/.alpha_insights.json` 并写入 `{"tikHubApiKey": "你的Key"}` |

### 🟡 可选扩展（需要配置）

| 数据源 | 说明 | 所需配置 |
|--------|------|---------|
| **知识库** | 历史研究报告、行业笔记 | Notion MCP / Confluence MCP |
...
SKILL 文件中的 SQL 示例使用 `{project}.{table_name}` 等占位符。配置了数据库 MCP 工具后，AI 会自动通过表搜索能力发现可用数据表，无需手动替换。
...
│   └── ...
├── resources/            # 执行资源（Stage 3-5 输入侧）
│   ├── data_sources.md
│   ├── research_engine.md
│   ├── judgment_rules.md
│   ├── quality_review.md # 独立质量复核（IQR）
│   └── anti_patterns.md
├── references/           # 报告标准（Stage 6-7 输出侧）
│   ├── report_standards.md
│   └── report_template.html
└── scripts/
    ├── report_helper.py  # ReportBuilder HTML 生成器
