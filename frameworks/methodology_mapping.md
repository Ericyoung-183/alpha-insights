# 方法论两层架构（Methodology Two-Tier Architecture）

> **定位**：定义方法论的使用规则，确保分析深度和严谨性
>
> **核心机制**：Tier 1（底层OS，始终运行）+ Tier 2（场景增强，自动触发）
>
> **关联模块**：`frameworks/_index.md`、`methodology/_index.md`
...
| 项目 | 说明 |
|------|------|
| **加载文件** | `methodology/mece.md` + `methodology/issue_tree.md` |
| **绑定阶段** | Stage 2：问题定义 |
| **核心作用** | 确保问题拆解完整（不重不漏），形成结构化的子问题树 |
| **检查点** | ① 维度是否穷尽？ ② 维度是否互斥？ ③ 是否拆解到可分析层级？ |

### 2. 假设驱动法 — Stage 3-4

| 项目 | 说明 |
|------|------|
| **加载文件** | `methodology/hypothesis_driven.md` |
...
| 项目 | 说明 |
|------|------|
| **加载文件** | `methodology/triangulation.md` |
...
| 项目 | 说明 |
|------|------|
| **加载文件** | `methodology/pyramid_principle.md` |
| **绑定阶段** | Stage 6：报告生成 |
...
| 项目 | 说明 |
|------|------|
| **加载文件** | `methodology/first_principles.md` |
| **应用阶段** | Stage 5：洞察生成 |
...
| 项目 | 说明 |
|------|------|
| **加载文件** | `methodology/pre_mortem.md` |
| **应用阶段** | Stage 5：洞察生成后 |
...
| 旧版设计 | 新版设计 | 变化原因 |
|---------|---------|---------|
| 框架级配对（每个框架配 1-3 个方法论） | Stage 级绑定（Tier 1）+ 场景级触发（Tier 2） | 消除两套逻辑的矛盾，简化执行 |
| 访谈法作为方法论映射 | 访谈法归属 Stage 3.5 工作流 | 访谈是信息获取方式，不是分析方法论 |
| 金字塔原理同时配对 Playing to Win | 金字塔原理仅绑定 Stage 6 | 消除定位矛盾，金字塔是表达方法，不是分析方法 |
| Tier 2 需用户选择接收/跳过 | Tier 2 自动引用 + 告知 | 降低用户决策负担，保持信息透明 |
